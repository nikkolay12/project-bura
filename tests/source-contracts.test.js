...
test("browser bundle identifies the v3.231 build and pins dependencies", () => {
  const html = read("index.html");
  assert.match(html, /v3\.231/);
  assert.match(html, /styles\.css\?v=3\.231\.0/);
  assert.match(html, /mobile\.css\?v=3\.231\.0" media="\(max-width: 660px\)"/);
  assert.match(html, /app\.js\?v=3\.231\.0/);
  assert.match(html, /@supabase\/supabase-js@2\.112\.3/);
  assert.match(html, /labels\.js\?v=3\.231\.0/);
  assert.match(html, /labelseng\.js\?v=3\.231\.0/);
  assert.match(html, /supabase-config\.js\?v=3\.231\.0/);
  assert.match(html, /sync-core\.js\?v=3\.231\.0/);
  assert.match(html, /bot-rules\.js\?v=3\.231\.0/);
  assert.match(html, /authoritative-client\.js\?v=3\.231\.0/);
});
...
test("login is isolated in a sidebar while games stay guest-first", () => {
  ...
  assert.match(html, /id="account-profile-rank"/);
  ...
  assert.match(html, /id="account-match-history"/);
  assert.match(html, /id="account-match-history-list"/);
  assert.match(html, /id="account-achievements"/);
  assert.match(html, /id="account-achievements-grid"/);
  assert.match(html, /id="account-coins-value"/);
  assert.match(html, /id="account-rank-value"/);
  assert.match(html, /id="account-karma-value"/);
  assert.match(html, /id="account-matches-value"/);
  ...
  assert.match(app, /function renderAccountMatchHistory\(\)/);
  assert.match(app, /function accountMatchOutcomeIcon\(result\)/);
  assert.match(app, /function renderAccountAchievements\(\)/);
  assert.match(app, /function accountWinStreakTone\(streak\)/);
  assert.match(app, /accountProgression\.currentWinStreak = won \? accountProgression\.currentWinStreak \+ 1 : 0;/);
  assert.match(app, /const LOCAL_PROFILE_PREVIEW_WIN_STREAKS = \[6, 8, 12\];/);
  assert.match(app, /type: "founder"/);
  assert.match(app, /type: "first100"/);
  assert.match(app, /match\.opponent = String\(state\.players/);
  assert.match(app, /accountProgression\.completedMatchesTotal \+= 1;/);
  ...
  assert.match(labels, /accountMatchHistory:/);
  assert.match(labels, /accountAchievements:/);
  assert.match(labels, /accountAchievementFounder:/);
  assert.match(labels, /accountAchievementFirstHundred:/);
  assert.match(englishLabels, /accountMatchHistory:/);
  assert.match(englishLabels, /accountAchievements:/);
  assert.match(englishLabels, /accountAchievementFounder:/);
  assert.match(css, /@keyframes account-achievement-gold-pulse/);
  ...
  assert.match(app, /function accountRankPoints\(\)/);
  assert.match(app, /function renderAccountProfileRank\(\)/);
  ...
  assert.match(app, /const ACCOUNT_PLAYER_NAME_LIMIT = 10;/);
  assert.match(app, /ID \$\{currentAccountPublicId\}/);
  assert.match(app, /function accountAvatarFromUser\(user\)/);
  assert.match(app, /async function saveAccountAvatar\(\)/);
  assert.match(app, /client\.auth\.updateUser\(\{ data: \{ bura_avatar: avatar \} \}\)/);
  assert.match(html, /id="account-emblem-picker"/);
  assert.match(html, /id="account-avatar-rank-picker"/);
  assert.match(html, /id="account-avatar-suit-choices"/);
  assert.match(html, /data-avatar-suit="spades"[\s\S]*?data-avatar-suit="clubs"[\s\S]*?data-avatar-suit="diamonds"[\s\S]*?data-avatar-suit="hearts"/);
  assert.doesNotMatch(html, /account-avatar-frame-picker/);
  assert.match(html, /maxlength="10"/);
  assert.match(css, /\.account-profile-avatar\s*\{/);
  assert.match(css, /\.account-avatar-suit-choices\s*\{/);
  assert.match(app, /const ACCOUNT_EMBLEM_PICKER_IDLE_MS = 5000;/);
  assert.match(app, /function resetAccountEmblemPickerTimer\(\)/);
  assert.match(css, /\.account-name-message\[hidden\]\s*\{/);
  assert.match(labels, /accountCustomizeEmblem:/);
  assert.match(englishLabels, /accountCustomizeEmblem:/);
  assert.match(app, /function localProfilePreviewEnabled\(\)/);
  ...
  assert.match(app, /elements\.accountMatchesValue\.textContent = String\(accountProgression\.completedMatchesTotal\);/);
  ...
  assert.match(app, /English copy reuses the Georgian label's presentation settings\./);
  assert.match(app, /const baseDefinition = window\.BURA_LABELS\?\.\[group\]\?\.\[key\];/);
